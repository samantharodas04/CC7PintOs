...
/* List of processes in sleep (wait) state (i.e. wait queue).  solut*/
...
void thread_awake (int64_t current_tick); /*solut */
void thread_priority_donate(struct thread *, int priority);/*solut */
/* Helper (Auxiliary) functions *//*solut*/
...
thread_init (void)
{
  ...
  initial_thread->sleep_endtick = 0; // a dummy value /*solut */
}
...
/* Called by the timer interrupt handler at each timer tick.
   ...
   the timer device. */
void
thread_tick (int64_t tick) /*solut */
{
  ...
  /* Wake any thread whose ticks_end has been expired. *//*solut */
  thread_awake(tick);
...
/* Wake up all sleeping threads whose ticks_end has been expired,
   ...
   This function must be called with interrupts turned off. *//*solut */
void
               ...
               thread_func *function, void *aux)
{
   ...
   * immediate context switching, for thread priority scheduling. *//*solut*/
...
thread_unblock (struct thread *t)
{
  ...
  // maintaining in the non-decreasing order of thread priority /*solut*/
  list_insert_ordered (&ready_list, &t->elem, comparator_greater_thread_priority, NULL);/*solut*/
  t->status = THREAD_READY;

  // ensure preemption : compare priorities of current thread and t (to be unblocked),/*solut*/
...
thread_priority_donate(struct thread *target, int new_priority)
{
  ...
  if (target == thread_current() && !list_empty (&ready_list)) {
    struct thread *next = list_entry(list_begin(&ready_list), struct thread, elem); /*solut*/
    if (next != NULL && next->priority > new_priority) {
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  t->priority = priority;
  t->original_priority = priority;/*solut */
  t->waiting_lock = NULL;/*solut */
  list_init (&t->locks);/*solut */
  t->sleep_endtick = 0;
...
/* Helper function: implementations */ /*solut*/
